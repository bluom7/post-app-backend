...
def send_otp(email, code):
    ...
    try:
        ...
        resend.Emails.send({
            "from": "POST App <onboarding@resend.dev>",
            "to": [email],
            "subject": "Your POST verification code",
            "html": f"<h2>POST</h2><p>Your code: <strong style='font-size:28px;letter-spacing:6px'>{code}</strong></p><p>Expires in 10 minutes.</p>"
        })
